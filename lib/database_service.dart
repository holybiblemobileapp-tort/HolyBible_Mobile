...
class DatabaseService {
  ...
  Future<List<BibleMatch>> search(String query) async {
    ...
    if (cleanQuery.contains(',')) {
      ...
      for (var p in parts) {
        ...
        if (loc != null) allLocs.addAll(await _searchByLocation(loc));
      }
      ...
      for (var m in results) {
        ...
        for (int i = 0; i <= words.length - queryWords.length; i++) {
          ...
          if (found) {
            ...
            allMatches.add(BibleMatch(phrase: verse.styledWords.sublist(sw - 1, ew).map((w) => w.text).join(' '), location: BibleLogic.formatLocation(verse.bookAbbreviation, verse.chapter, verse.verse, sw, ew), verse: verse, startWord: sw, endWord: ew));
            i = ew - 1;
          }
        }
      }
  ...
  Future<List<VectorSpaceRow>> getVectorSpaceGrid(String query, {int n = 3, bool includeBefore = true, bool includeAfter = true, int? limit, int? offset}) async {
    ...
    if (offset != null && offset < baseMatches.length) baseMatches = baseMatches.skip(offset).toList();
    if (limit != null && limit < baseMatches.length) baseMatches = baseMatches.take(limit).toList();
    for (var m in baseMatches) { matrix[m.location] = VectorSpaceRow(word: m.phrase, location: m.location); }
              ...
              if (contextMatches.length > 1) {
                for (var m in contextMatches.take(50)) {
                  ...
                  if (targetStart <= m.verse.wordCount) {
                    ...
                    for (int i = targetStart; i <= m.verse.wordCount; i++) { targetEnd = i; if (m.verse.styledWords[i - 1].text.contains(_terminalPunct)) break; }
                    matrix[match.location]!.witnessesBefore.add(WitnessPair(witness: m, spiritual: BibleMatch(phrase: m.verse.styledWords.sublist(targetStart - 1, targetEnd).map((w) => w.text).join(' '), location: BibleLogic.formatLocation(m.verse.bookAbbreviation, m.verse.chapter, m.verse.verse, targetStart, targetEnd), verse: m.verse, startWord: targetStart, endWord: targetEnd)));
                  }
                }
                foundBest = true;
              }
            ...
            if (phrase.trim().isNotEmpty) {
              ...
              if (contextMatches.length > 1) {
                for (var m in contextMatches.take(50)) {
                  ...
                  if (targetEnd >= 1) {
                    int targetStart = 1;
                    for (int i = targetEnd; i >= 1; i--) { if (m.verse.styledWords[i - 1].text.contains(_terminalPunct)) { targetStart = i + 1; break; } targetStart = i; }
                    matrix[match.location]!.witnessesAfter.add(WitnessPair(witness: m, spiritual: BibleMatch(phrase: m.verse.styledWords.sublist(targetStart - 1, targetEnd).map((w) => w.text).join(' '), location: BibleLogic.formatLocation(m.verse.bookAbbreviation, m.verse.chapter, m.verse.verse, targetStart, targetEnd), verse: m.verse, startWord: targetStart, endWord: targetEnd)));
                  }
                }
                foundBest = true;
              }
    ...
    return matrix.values.toList()..sort((a,b) => a.bookOrder.compareTo(b.bookOrder));
  }
  ...
  Future<Map<String, String>> getParenthesesMap() async {
    try {
      ...
      final List<dynamic> data = json.decode(jsonString);
      return { for (var item in data) item['AuxVerb'].toString().toLowerCase().replaceAll(RegExp(r'[.,;:!?¶\(\)\[\]]'), '').trim() : item['Symbol'].toString() };
    } catch (_) { return {}; }
  }
  ...
  Future<BibleVerse?> getDailyVerse() async {
    ...
    return maps.isEmpty ? null : BibleVerse.fromJson(maps.first);
  }
  ...
  Future<void> addLibraryChapter(String book, String chapter, int index, String content, List<String> vectors) async {
    ...
    await db.insert('library', { 'book_title': book, 'chapter_title': chapter, 'chapter_index': index, 'content': content, 'source_vectors': vectors.join(','), 'created_at': now });
  }
}
